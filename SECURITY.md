...
## Agora's trust model — read this before deploying

**Agora has no user accounts and no in-app permission system.** Anyone who can
...
- **Do not expose a board to the internet unauthenticated.** `AGORA_AUTH=none`
  is meant for localhost. In production, set `AGORA_AUTH=password`, or put Agora
...
- **`AGORA_AUTH=proxy` is only as good as your network.** It trusts an HTTP
  ...
  header can be forged. Agora refuses the header unless the peer address is in
...
- **API tokens are bearer credentials.** Anything holding one has full board
  access. They must be at least 32 characters; Agora refuses shorter ones at
...
- **Webhook payloads are signed, not encrypted.** Verify the
  `X-Agora-Signature` HMAC on your receiver, and serve it over TLS.

## Supported versions

Agora is pre-1.0. Security fixes land on `main` and in the next release; there
